echo " "
echo "IIUG 2019 Kernel Lab AYSTATS2 script"
echo " "
...
create index jcind on jctab (col1);
END

# Determine the partnum of our jcind index
PARTNUM=`oncheck -pt jc:jctab | grep "Partition partnum" | tail -1 | awk '{print $3}'`
...
PGNUM=`oncheck -pp $PARTNUM 10 | grep BTREE | awk '{print $1}' | cut -c3-10`
...
then
    ...
    echo "Otherwise interrupt it and get your instructor :)"
    read a
    tbpatch -d $ROOTPATH -o $PGNUM -b 0x96 -v 0xffffffff
